...
# ════════════════════════════════════════════════════════════════════
# RESUME BLOCK — adapt these two variables before running
# ════════════════════════════════════════════════════════════════════

# Path to the checkpoint file you want to resume from
CHECKPOINT_PATH = "checkpoints/ckpt_ep330000.pt"

# Folder where new checkpoints and the reward CSV will be written
# Use the same folder as the original run to continue the sequence
CHECKPOINT_DIR  = "checkpoints"

# Total target episodes (same as original run)
TOTAL_EPISODES  = 700_000

# ════════════════════════════════════════════════════════════════════
# 1. Recreate network and optimizer with identical architecture
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
# 2. Load checkpoint — restores weights, optimizer state, and metadata
# ════════════════════════════════════════════════════════════════════
...
print(f"Reward entries loaded: {len(rewards_so_far)}")
...
if n_episodes_remaining <= 0:
    ...
    # ════════════════════════════════════════════════════════════════
    # 4. Reopen reward CSV in APPEND mode so history is preserved
    #    train_rl_agent opens it internally in "w" mode, so we
    #    patch the path to a continuation file to avoid overwriting.
    #    Alternatively we pass a flag — here we use a suffixed name.
    # ════════════════════════════════════════════════════════════════
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    reward_log_path = os.path.join(
        CHECKPOINT_DIR,
        f"episode_rewards_resumed_from_ep{current_episode_resumed}.csv"
    )

    # Monkey-patch train_rl_agent to open the CSV in append mode
    # by writing a thin wrapper that opens the file before calling
    # the function — simpler than modifying train_rl_agent itself.

    import builtins
    _original_open = builtins.open
    _reward_log_patched = False

    def _patched_open(path, mode="r", **kwargs):
        """Intercept the reward CSV open and force append mode."""
        global _reward_log_patched
        if "episode_rewards" in str(path) and mode == "w" and not _reward_log_patched:
            _reward_log_patched = True
            # Write header only if file does not exist yet
            file_exists = os.path.exists(reward_log_path)
            f = _original_open(reward_log_path, "a", **kwargs)
            if not file_exists:
    ...
    episode_rewards, episode_losses, total_steps, final_episode = train_rl_agent(
        q_net=q_net,
        optimizer=optimizer,
        loss_fn=loss_fn,
        n_episodes=n_episodes_remaining,
        d_mainline=d_mainline,
        d_ramp=d_ramp,
        sample_cells=sample_cells,
        gamma=0.95,
        epsilon_start=epsilon_resumed,     # resume from saved epsilon
        epsilon_end=0.05,
        epsilon_decay=0.99999,
        k_reward=-1.0,
        lr_threshold=lr_threshold,
        lr_low=lr_low,
        total_steps=total_steps_resumed,   # resume step count for lr switch
        start_episode=current_episode_resumed,  # resume episode count
    ...
    print(f"Training complete. Final episode: {final_episode}")
