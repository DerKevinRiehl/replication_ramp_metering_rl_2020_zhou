...
# lr_threshold: episode count at which α drops from 0.05 → 0.01 (paper §4.1)
lr_high      = 0.05    # α for first 100 000 steps  (paper §4.1)
lr_low       = 0.01    # α afterwards               (paper §4.1)
lr_threshold = 100_000  # step count at which α switches
...
def train_rl_agent(
    ...
    file_exists = os.path.exists(reward_log_path)
    reward_file = open(reward_log_path, "a")  # append mode always
    if not file_exists:
        reward_file.write("episode,reward\n")  # header only on first creation
...
# ════════════════════════════════════════════════════════════════════════════
# RESUME + BRUTEFORCE FIND BEST
# Resumes training from a checkpoint, evaluates greedily every N episodes,
# and saves the best-performing network found.
# ════════════════════════════════════════════════════════════════════════════

# ── Settings — adapt before running ─────────────────────────────────────────
CHECKPOINT_PATH   = "checkpoints/ckpt_ep250000.pt"  # checkpoint to resume from
CHECKPOINT_DIR    = "checkpoints"
TOTAL_EPISODES    = 1_100_000   # run until this episode count
GREEDY_EVAL_EVERY = 1           # evaluate greedily every N episodes (1=every ep)
REWARD_TARGET     = -2049.51     # stop early if greedy reward exceeds this value
                                 # set to -np.inf to disable early stopping

# ── Re-use run_trained_policy for greedy evaluation ──────────────────────────
def greedy_eval(q_net, d_main, d_ramp_arr, sample_cells):
    """Run policy with epsilon=0 and return total episode reward."""
    res = run_trained_policy(q_net, d_main, d_ramp_arr, sample_cells)
    rho_ctrl = res["rho"][:, control_cell] / n_lanes[control_cell]
    return float(np.sum(-1.0 * np.abs(rho_ctrl - rho_des)))

# ── 1. Recreate network and optimizer ────────────────────────────────────────
q_net     = QNetwork(n_features=140, n_hidden=420, n_actions=len(all_actions))
optimizer = optim.SGD(q_net.parameters(), lr=lr_high)
loss_fn   = nn.MSELoss()

# ── 2. Load checkpoint ────────────────────────────────────────────────────────
epsilon_resumed, total_steps_resumed, current_episode_resumed, \
    rewards_so_far, losses_so_far = load_checkpoint(
        CHECKPOINT_PATH, q_net, optimizer
    )

print(f"Resumed from episode : {current_episode_resumed}")
print(f"Total steps so far   : {total_steps_resumed}")
print(f"Epsilon at resume    : {epsilon_resumed:.6f}")

# ── 3. Compute remaining episodes ────────────────────────────────────────────
n_episodes_remaining = TOTAL_EPISODES - current_episode_resumed

if n_episodes_remaining <= 0:
    print("Training already complete — nothing to resume.")
else:
    print(f"Resuming for {n_episodes_remaining} more episodes "
          f"({current_episode_resumed} → {TOTAL_EPISODES})")
    print(f"Greedy evaluation every {GREEDY_EVAL_EVERY} episode(s).")
    print(f"Early stopping if greedy reward > {REWARD_TARGET}")
    print()

    os.makedirs(CHECKPOINT_DIR, exist_ok=True)

    # Best greedy reward tracking
    best_greedy_reward  = -np.inf
    best_greedy_episode = None
    best_ckpt_path      = os.path.join(CHECKPOINT_DIR, "ckpt_best_greedy.pt")

    # Greedy reward log
    greedy_log_path = os.path.join(CHECKPOINT_DIR, "greedy_rewards.csv")
    greedy_file     = open(greedy_log_path, "a")
    if os.path.getsize(greedy_log_path) == 0:
        greedy_file.write("episode,greedy_reward\n")

    # Reward CSV — append mode
    reward_log_path = os.path.join(CHECKPOINT_DIR, "episode_rewards.csv")
    reward_file_exists = os.path.exists(reward_log_path)
    reward_file = open(reward_log_path, "a")
    if not reward_file_exists:
        reward_file.write("episode,reward\n")

    # Training state
    epsilon      = epsilon_resumed
    total_steps  = total_steps_resumed
    lr_switched  = (current_episode_resumed >= lr_threshold)
    early_stop   = False

    for ep in range(n_episodes_remaining):
        current_episode = current_episode_resumed + ep

        # LR switch at episode level
        if not lr_switched and current_episode >= lr_threshold:
            optimizer.param_groups[0]["lr"] = lr_low
            lr_switched = True
        ...
        for k in range(len(d_mainline) - 1):
            state = extract_rl_state(sim_state, sample_cells, prev_ramp_arrival)
            action_value, action_idx = select_action_epsilon_greedy(
                state, epsilon, q_net
            )
            next_sim_state = ctm_step(
                sim_state=sim_state,
            ...
            next_state = extract_rl_state(
                next_sim_state, sample_cells, sim_state["r_in"]
            )
            reward = compute_reward(next_state, k_reward=-1.0)
            train_step(
                q_net=q_net, optimizer=optimizer, loss_fn=loss_fn,
                state=state, action_idx=action_idx,
                reward=reward, next_state=next_state, gamma=0.95
            )
            total_reward      += reward
            total_steps       += 1
            prev_ramp_arrival  = sim_state["r_in"]
        ...
        # ── Greedy evaluation every GREEDY_EVAL_EVERY episodes ────────────
        if (ep + 1) % GREEDY_EVAL_EVERY == 0:
            greedy_reward = greedy_eval(q_net, d_mainline, d_ramp, sample_cells)

            greedy_file.write(f"{current_episode + 1},{greedy_reward:.4f}\n")
            greedy_file.flush()

            if greedy_reward > best_greedy_reward:
                best_greedy_reward  = greedy_reward
                best_greedy_episode = current_episode + 1
                # save best network — overwrite same file each time
                torch.save({
                    "model_state":     q_net.state_dict(),
                    "optimizer_state": optimizer.state_dict(),
                    "epsilon":         epsilon,
                ...
                print(f"  ★ New best greedy reward: {best_greedy_reward:.4f} "
                      f"at episode {best_greedy_episode} — saved to {best_ckpt_path}")

            # early stopping
            if best_greedy_reward > REWARD_TARGET:
                print(f"\nEarly stop: greedy reward {best_greedy_reward:.4f} "
                      f"> target {REWARD_TARGET} at episode {current_episode + 1}")
                early_stop = True

        # Periodic checkpoint every 10k episodes
        if (current_episode + 1) % 10_000 == 0:
            fname = os.path.join(
                CHECKPOINT_DIR, f"ckpt_ep{current_episode + 1}.pt"
            )
            torch.save({
                "model_state":     q_net.state_dict(),
                "optimizer_state": optimizer.state_dict(),
        ...
        if (ep + 1) % 1000 == 0:
            print(
                ...
                f"best greedy = {best_greedy_reward:.4f} (ep {best_greedy_episode}) | "
    ...
    print()
    print("=" * 60)
    print(f"  BEST GREEDY CHECKPOINT : {best_ckpt_path}")
    print(f"  BEST GREEDY EPISODE    : {best_greedy_episode}")
    print(f"  BEST GREEDY REWARD     : {best_greedy_reward:.4f}")
    print("=" * 60)
